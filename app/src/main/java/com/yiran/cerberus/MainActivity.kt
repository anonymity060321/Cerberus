    ...
    Column(
        ...
        Surface(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                TermItem("支持 Steam 协议", "使用已导入的 shared_secret 在本机生成 Steam 登录验证码；shared_secret 与验证码均不会上传。")
                TermItem("通行密钥", "支持将 Cerberus 设为系统凭据提供程序。私钥由本机安全硬件保护，不会写入加密备份或上传；使用时仅进行必要的在线安全验证。")
                TermItem("账号密码自动填充", "仅在您主动选择并通过主密码或已启用的生物识别后，Cerberus 才会把选定的账号密码交给系统填入当前登录表单。")
            }
        }
